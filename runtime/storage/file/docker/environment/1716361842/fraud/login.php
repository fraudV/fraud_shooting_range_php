...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $ipc = ipCount(3);
    if ($ipc){
        $message = "操作次数过多，请1分钟后重试";
    }else{
        $user = isset($_POST['user']) ? $_POST['user'] : '';
        $pwd = isset($_POST['pwd']) ? $_POST['pwd'] : '';
        $hold = isset($_POST['hold']) ? $_POST['hold'] : '';
        ...
        if ($result){
            while ($row = $result->fetch_assoc()) {
                if ($row['pwd'] === $pwd) {
                    if($hold==='on'){
                        $base = $user . ':' . $pwd;
                        setcookie("token",  base64_encode($base));
                    }else{
                        setcookie("user", $user, time()+600);
                        setcookie("pwd", $user, time()+600);
                    }
...
<form class="layui-form" method="post">
    <div class="demo-login-container">
        ...
        <div class="layui-form-item">
            <input type="checkbox" name="hold" lay-skin="primary" title="保持登录">
        </div>
        <div class="layui-form-item">
